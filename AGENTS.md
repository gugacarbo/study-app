...
- **Plugin required**: [Obsidian Local REST API](https://github.com/coddingtonbear/obsidian-local-rest-api) — install via Community Plugins or BRAT
- **Architecture**: Server functions call Obsidian REST API at `http://localhost:27124` (local-only, not available in production Cloudflare Workers)
- **Memory structure** in vault (`valut/study-app/`):
  - `memory/sessions/` — Quiz session logs (auto-saved on completion)
  - `memory/profile.md` — Learning profile (topics studied, strong/weak areas)
  - `memory/topics/` — Per-topic notes generated by AI
  - `questions/` — Question bank exports as Markdown
  - `stats/` — Learning stats snapshots
- **AI context injection**: Before AI calls (quiz generation, ingestion, explanations), the system queries Obsidian vault for relevant notes and injects them as context into the system prompt
- **Source files**:
  - `src/lib/obsidian.ts` — REST API client (read/write/search)
  - `src/lib/memory.ts` — Memory manager (save sessions, build context, manage profile)
  - `src/server-functions/obsidian.ts` — Server functions (status, save, export, search)
  - `src/components/ObsidianConfigForm.tsx` — Connection config UI
  - `src/components/ObsidianPanel.tsx` — Main management panel
  - `src/routes/obsidian.tsx` — Route page
