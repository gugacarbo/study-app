...
## Overview
Single-user web app for studying college exams using previous exams as source material. Upload PDFs, extract questions via AI, interactive quiz mode, progress tracking.

## TanStack CLI Command Used
```bash
npx @tanstack/cli@latest create study-app-temp --deployment cloudflare --agent
```

## Stack
- TanStack Start (SPA mode)
- TanStack Router, AI, Query, Form, Store, Table, Hotkeys, Virtual, CLI, Intent
...
- Add TanStack Table for stats page (currently using plain HTML table)
- Add TanStack Hotkeys for quiz keyboard shortcuts (currently using manual event listeners)
