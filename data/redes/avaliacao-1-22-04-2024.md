...
| **📝 Avaliação**         | Boa, com ressalvas. Na L2 (Enlace), o aluno citóu "endereçamento lógico". Endereçamento lógico (IP) é responsabilidade da camada de Rede (L3). A camada de Enlace lida com endereçamento físico (MAC).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
## Questão 9 — CSMA/CA no IEEE 802.11 (WiFi)

> Descreva o funcionamento do mecanismo de acesso aleatório CSMA/CA implementado no IEEE 802.11 (WiFi).

| Aspecto                  | Conteúdo                                                                                                                                                                                                                                                                                              |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **📖 Resposta Ideal**    | Como no Wi-Fi não é possível detectar colisões facilmente, usa-se o CA (_Collision Avoidance_). A estação escuta o meio; se estiver livre, aguarda um tempo e transmite. Se estiver ocupado, realiza um "backoff" (espera um tempo aleatório) antes de tentar novamente. Exige ACKs para confirmação. |
| **✍️ Resposta do Aluno** | "Protocolo para evitar colisão de pacotes, por meio da leitura do meio em intervalos de tempos aleatórios procurando uma brecha para enviar os dados."                                                                                                                                                |
| **📝 Avaliação**         | Parcialmente correta. Faltou detalhar a necessidade do uso de ACKs e o mecanismo exato de recuo exponencial (_backoff_), mas a essência do "escutar para evitar" está presente.                                                                                                                       |
| **💡 Explicação**        | O CSMA/CA prioriza evitar colisões no meio sem fio: escuta, espera, recua aleatoriamente e confirma com ACK. Sem ACK, o emissor não sabe se o quadro chegou corretamente.                                                                                                                             |

---

## Questão 10 — Estação Oculta, Estação Exposta e RTS/CTS

> Explique os fenômenos da "estação oculta" e da "estação exposta" ... e como o protocolo CTS/RTS pode auxiliar.

| Aspecto                  | Conteúdo                                                                                                                                                                                                                                                                                                      |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **📖 Resposta Ideal**    | Oculta: A e C querem enviar para B, mas não se enxergam, causando colisão em B. Exposta: B quer enviar para A e acha que o meio está ocupado porque escuta C enviando para D, perdendo oportunidade de transmitir. RTS/CTS reserva o meio através de pacotes curtos, avisando todos ao redor para aguardarem. |
| **✍️ Resposta do Aluno** | "Estação oculta: quando duas estações não se veem e transmitem para o mesmo destino. Estação exposta: quando uma estação acha que o meio está ocupado por ouvir outra. O RTS/CTS reserva o canal para evitar isso."                                                                                           |
| **📝 Avaliação**         | Razoável. A estação exposta é um conceito notoriamente difícil de colocar em palavras na hora da prova, o que explica a nota parcial (0,5).                                                                                                                                                                   |
| **💡 Explicação**        | Estações ocultas causam colisão no receptor comum; estações expostas desperdiçam oportunidades de transmissão. O RTS/CTS reduz esses efeitos ao reservar o canal antes do envio dos dados.                                                                                                                    |

```mermaid
flowchart LR
  A[Estacao A] -->|Dados para B| B[Estacao B]
  C[Estacao C] -->|Dados para B| B
  A -.Nao escuta C.- C
  A -->|RTS| B
  B -->|CTS| A
  B -->|CTS| C
