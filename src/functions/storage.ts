...
async function getBucket(
	...
	const workersEnv = await getBucketFromCloudflareWorkersModule();
	if (workersEnv?.[binding]) return workersEnv[binding];

	const cf = globalThis as {
		cloudflare?: { env?: Partial<Record<BucketBinding, R2Bucket>> };
	};
	return cf.cloudflare?.env?.[binding];
}
...
export async function requireFilesBucket(
	ctx?: ServerFnContext,
): Promise<R2Bucket> {
	const bucket = await getFilesBucket(ctx);
	if (!bucket) {
