...
} from "@/features/ai/agents/improve-options";
...
const improveOptionsRequestSchema = z.object({
...
async function runImproveOptions(
	questionId: number,
	...
	const run = agentRuns.createRun(IMPROVE_OPTIONS_STAGE_ID, "Improve Options");

	const resolvedTools = resolveToolsForAgent({
		agent: "improve_options",
		config,
		context: {
			queries,
			providerConfig,
...
export const Route = createFileRoute("/api/improve-options/")({
	server: {
		handlers: {
			POST: async ({ request }: { request: Request }) => {
				const payloadRaw = await request.json().catch(() => null);
				const parsed = improveOptionsRequestSchema.safeParse(payloadRaw);
				if (!parsed.success) {
					return new Response(
						JSON.stringify({
							error: "Invalid improve-options payload",
				...
				const stream = new ReadableStream<Uint8Array>({
					start(controller) {
						...
						void (async () => {
							try {
								await runImproveOptions(parsed.data.questionId, send);
							} catch (error) {
								console.error(
									`[${new Date().toISOString()} ERROR improve-options-handler] Improve options failed:`,
									error,
