...
export function useChatClient(activeId: string | null) {
	...
	useEffect(() => {
		...
		// Load persisted metrics and totals for this conversation
			...
			onLoadingChange: (loading) => {
				...
				if (!loading) {
					...
					setAssistantMetrics((prev) => {
						...
						if (lastId) {
							const runtime = perfRuntimeRef.current[lastId];
	...
	// Persist per-message metrics to store on every change
	useEffect(() => {
		if (activeId) {
			saveAssistantMetrics(activeId, assistantMetrics);
		}
	}, [assistantMetrics, activeId]);

	// Persist accumulated token totals to store on every change
	useEffect(() => {
		if (activeId) {
			saveTokenTotals(activeId, chatTokenTotals);
		}
