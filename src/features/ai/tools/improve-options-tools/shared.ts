...
export const IMPROVE_OPTIONS_TOOL_ERROR_CODE = "IMPROVE_OPTIONS_TOOL_ERROR";
...
const improveOptionsOptionsSchema = z
...
const improveOptionsAnswersSchema = z
	.array(z.string().trim().min(1))
...
function readImproveOptionsAnswers(input: unknown): string[] {
...
function preprocessImproveOptionsPatch(input: unknown): unknown {
	...
	const answers = readImproveOptionsAnswers(input);
...
export const improveOptionsToolFailureSchema = z.object({
	ok: z.literal(false),
	error: z.object({
		code: z.string(),
		message: z.string(),
...
export { improveOptionsAnswersSchema, improveOptionsOptionsSchema };
