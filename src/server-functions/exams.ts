...
export const generateExamQuestionExplanations = createServerFn({ method: 'POST' })
  .inputValidator(
    z.object({
  ...
  .handler(async (ctx) => {
    ...
    if (!apiKey) {
      throw new Error('AI API key not configured. Please configure it in /config first.');
    }
    ...
    const requestedIds = ctx.data.questionIds ? new Set(ctx.data.questionIds) : null;
