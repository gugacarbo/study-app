...
async function getDBFromCloudflareWorkersModule(): Promise<D1Database | undefined> {
  if (cachedWorkersEnv === undefined) {
    try {
      const mod = (await import('cloudflare:workers')) as CloudflareWorkersModule;
...
export async function getDB(ctx?: ServerFnContext): Promise<D1Database | undefined> {
