...
2. Sem sessão → 401.
3. Sessão sem permissão `app:use` em D1 → **401** (`requireSession` — ADR-0004).
4. Com sessão + `app:use` → `userId = session.user.id` em todas as queries.
...
| 16  | sessão válida mas sem `app:use` em `user_roles`   | **401** em server functions (`requireSession`)  |
...
```text
npm run typecheck                                                                    # exit 0
npm test -- src/lib/auth.test.ts src/lib/auth-allowed-email-domain.test.ts           # 4 passed
npm test -- src/lib/rbac-bootstrap.test.ts src/routes/login.spec.tsx src/functions/auth/require-session.test.ts  # 7 passed
```
