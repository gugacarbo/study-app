...
describe("computeQuestionChanges", () => {
	...
	it("detects question stem changes", () => {
		const original = makeQuestion({
			id: 1,
		...
		expect(changes[0]).toMatchObject({
			...
			label: "Question stem",
			before: "What is 2 + 2?",
			after: "What is the sum of 2 and 2?",
			decision: "pending",
		});
	});
...
describe("resolveQuestion", () => {
	it("reverts question stem while keeping other pending changes", () => {
		const original = makeQuestion({
			id: 1,
			question: "Old stem?",
			explanation: "Old",
		...
		expect(resolved.question).toBe("Old stem?");
	...
	it("uses draft values for pending decisions", () => {
		...
		expect(resolved.question).toBe("New stem?");
