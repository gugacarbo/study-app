...
import { improveSingleQuestion } from "@/features/ai/agents/improve-options/improve-single-question";
	...
	it("improves a question through workspace tools and emits workspace updates", async () => {
		...
		expect(result.agentRun).toEqual(
			expect.objectContaining({
				...
				userPrompt: expect.stringContaining(
					"Improve options for question #7.",
				),
			}),
		);
		expect(streamChatMessagesMock).toHaveBeenCalledWith(
			...
			[
				{
					role: "user",
					content: expect.stringContaining("Improve options for question #7."),
				},
			],
			expect.objectContaining({
